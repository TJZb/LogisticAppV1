...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['fuel_record_id'])) {
    ...
    $mileage = isset($_POST['odometer_reading']) ? $_POST['odometer_reading'] : null;
    ...
    $stmt_before = $conn->prepare("SELECT TOP 1 odometer_reading FROM fuel_records WHERE vehicle_id = (SELECT vehicle_id FROM fuel_records WHERE fuel_record_id = ?) AND fuel_record_id <> ? AND odometer_reading IS NOT NULL AND fuel_date < ? ORDER BY fuel_date DESC");
    ...
    $stmt_after = $conn->prepare("SELECT TOP 1 odometer_reading FROM fuel_records WHERE vehicle_id = (SELECT vehicle_id FROM fuel_records WHERE fuel_record_id = ?) AND fuel_record_id <> ? AND odometer_reading IS NOT NULL AND fuel_date > ? ORDER BY fuel_date ASC");
    ...
    if (isset($_POST['reject'])) {
        ...
        $stmt_last = $conn->prepare("SELECT TOP 1 odometer_reading FROM fuel_records WHERE vehicle_id = (SELECT vehicle_id FROM fuel_records WHERE fuel_record_id = ?) AND fuel_record_id <> ? AND odometer_reading IS NOT NULL AND fuel_date <= ? ORDER BY fuel_date DESC");
...
<body class="min-h-screen bg-gradient-to-br from-[#1e293b] to-[#0f172a] text-[#e0e0e0]">
                            ...
                            <form method="post" class="space-y-3 mt-2">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" step="0.01" name="price_per_liter" id="price_per_liter_<?= $order['fuel_record_id'] ?>" class="w-full rounded px-3 py-2 text-[#111827]" value="<?= htmlspecialchars($order['price_per_liter']) ?>" required oninput="calcVolume<?= $order['fuel_record_id'] ?>()">
                                    </div>
                                    <div>
                                    ...
                                    <?php if (!empty($order['trailer_vehicle_id'])) : ?>
                                    <div>
                                        <label class="block font-semibold mb-1">เลขไมล์รถพ่วง (คำนวณอัตโนมัติ)</label>
                                        <div class="text-xs text-blue-300 mb-2">
                                            <?php if ($trailer_current_mileage !== false && $trailer_current_mileage > 0) : ?>
                                                ⚠️ เลขไมล์ปัจจุบันของรถพ่วง: <?= number_format($trailer_current_mileage) ?> กม.
                                            <?php else : ?>
                                                ⚠️ ไม่พบข้อมูลเลขไมล์รถพ่วง
                                            <?php endif; ?>
                                        </div>
                                        <input type="number" name="trailer_calculated_mileage" id="trailer_calculated_mileage_<?= $order['fuel_record_id'] ?>" class="w-full rounded px-3 py-2 text-[#111827] bg-gray-200" placeholder="จะคำนวณอัตโนมัติเมื่อยืนยัน" readonly>
                                        <div class="text-xs text-gray-400 mt-1">เลขไมล์ปัจจุบันของรถพ่วง + ระยะวิ่งรถหลัก</div>
                                    </div>
                                    <?php endif; ?>
                                    ...
                                    <div>
                                        ...
                                        <input type="number" name="odometer_reading" id="odometer_reading_<?= $order['fuel_record_id'] ?>" class="w-full rounded px-3 py-2 text-[#111827]" value="<?= htmlspecialchars($order['odometer_reading']) ?>" required oninput="updateTrailerMileage<?= $order['fuel_record_id'] ?>()" <?php if ($initial_min_mileage !== false && $initial_min_mileage > 0) : ?>min="<?= $initial_min_mileage ?>" <?php endif; ?> <?php if ($initial_max_mileage !== false) : ?>max="<?= $initial_max_mileage ?>" <?php endif; ?>>
                                    </div>
                                </div>
                                <div>
    ...
    <script>
        ...
        // Define all modal functions
        <?php if (!empty($orders)) : ?>
            <?php foreach ($orders as $order) : ?>
                window.toggleOtherFuelType<?= $order['fuel_record_id'] ?> = function() {
                ...
                window.calcVolume<?= $order['fuel_record_id'] ?> = function() {
                    var cost = parseFloat(document.getElementById('price_per_liter_<?= $order['fuel_record_id'] ?>').value) || 0;
                ...
                window.updateTrailerMileage<?= $order['fuel_record_id'] ?> = function() {
                    <?php if (!empty($order['trailer_vehicle_id'])) : ?>
                        var mainMileage = parseFloat(document.getElementById('odometer_reading_<?= $order['fuel_record_id'] ?>').value) || 0;
                        var trailerCurrentMileage = <?= ($trailer_current_mileage !== false && $trailer_current_mileage > 0) ? $trailer_current_mileage : 0 ?>;
                        
                        if (mainMileage > 0 && trailerCurrentMileage > 0) {
                            // ดึงเลขไมล์ก่อนหน้าของรถหลักเพื่อคำนวณระยะวิ่ง
                            fetch('get_mileage_constraint.php', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/x-www-form-urlencoded',
                                },
                                body: 'vehicle_id=<?= $order['vehicle_id'] ?>&fuel_record_id=<?= $order['fuel_record_id'] ?>&fuel_date=' + encodeURIComponent(document.getElementById('fuel_date_<?= $order['fuel_record_id'] ?>').value.replace('T', ' '))
                            })
                            .then(response => response.json())
                            .then(data => {
                                if (data.success && data.min_mileage !== null) {
                                    var distance = mainMileage - data.min_mileage;
                                    if (distance > 0) {
                                        var newTrailerMileage = trailerCurrentMileage + distance;
                                        document.getElementById('trailer_calculated_mileage_<?= $order['fuel_record_id'] ?>').value = newTrailerMileage.toLocaleString('th-TH');
                                    }
                                }
                            })
                            .catch(error => {
                                console.error('Error calculating trailer mileage:', error);
                            });
                        }
                    <?php endif; ?>
                }

                window.updateMileageConstraint<?= $order['fuel_record_id'] ?> = function() {
                    ...
                    fetch('get_mileage_constraint.php', {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                var mileageInput = document.getElementById('odometer_reading_<?= $order['fuel_record_id'] ?>');
                ...
                window.validateReject<?= $order['fuel_record_id'] ?> = function() {
                    ...
                    document.getElementById('odometer_reading_<?= $order['fuel_record_id'] ?>').required = false;
                    return confirm('ยืนยันการยกเลิกรายการนี้?\nหมายเหตุ: ' + notes);
                }

                window.openOrderModal<?= $order['fuel_record_id'] ?> = function() {
                    ...
                    document.getElementById('odometer_reading_<?= $order['fuel_record_id'] ?>').required = true;
                    // คำนวณเลขไมล์รถพ่วงเมื่อเปิด modal
                    updateTrailerMileage<?= $order['fuel_record_id'] ?>();
                }

                window.closeOrderModal<?= $order['fuel_record_id'] ?> = function() {
